...
def usage():
    ...
    print "-e --execute=file_to_run - execute the given file upon receiving a conenction"
    print "-c --command             - Initialize a command shell"
    print "-u --upload=destination  - upon receiving connection upload a file and write to [destination]"
    print
    print
    print "Examples:    "
    print "bhpnet.py -t 192.168.0.1 -p 555 -l -c"
...
def run_command(command):
    ...
    # run the command and get the otput back
    try:
...
def client_handler(client_socket):
    ...
    if command:
        while True:
            ...
            # now we receive untill we see a linefeed (enter key)
            cmd_buffer = ""
            while "\n" not in cmd_buffer:
